...
def init_db():
    ...
    cursor.execute('''CREATE TABLE users(
                        ...
                        which_sport_do_you_do TEXT NOT NULL,
...
def signup():
    if request.method == 'POST':
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO users (name, email, password, username, age, gender, which_sport_do_you_do) VALUES (?, ?, ?, ?, ?, ?, ?)",
            ...
            return redirect(url_for('tell_us_about_yourself'))
...
def tell_us_about_yourself():
    ...
    if request.method == 'POST':
        ...
        which_sport_do_you_do = request.form['sports']
    ...
    return render_template('tusp.html')
...
def dashboard():
    ...
    conn = sqlite3.connect('users.db')
    cursor = conn.cursor()
    cursor.execute("SELECT name, username, age, gender FROM users WHERE email = ?", (email,))
    user = cursor.fetchone()
    conn.close()

    if user:
        name, username, age, gender = user
        return render_template(
            'db.html', name=name, username=username, age=age, gender=gender
        )
    else:
...
response_scores = {"Never": 10, "Rarely": 7, "Sometimes": 4, "Often": 1}
...
def quiz(disease):
    ...
    if request.method == "POST":
        total_score = 0
        max_score = len(random_questions) * 10 
        for i, (question, _) in enumerate(random_questions):
            answer = request.form.get(f"question_{i}", "Never")
            total_score += response_scores.get(answer, 0)
        
        health_percentage = (total_score / max_score) * 100
        return render_template("quiz_result.html", health_percentage=health_percentage)

    return render_template("quiz.html", quiz=selected_quiz, questions=random_questions)

@app.route('/quiz/brain-test')
def home():
    return redirect(url_for('quiz', disease='brain_test'))
