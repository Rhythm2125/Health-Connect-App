...
def signup():
    if request.method == 'POST':
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO users (name, email, password, age, gender, which_sport_do_you_do) VALUES (?, ?, ?, ?, ?, ?)",
...
def tell_us_about_yourself():
    ...
    if request.method == 'POST':
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE users SET age = ?, gender = ?, which_sport_do_you_do = ? WHERE name = ?",
            (age, gender, which_sport_do_you_do, email))
        conn.commit()
        conn.close()
...
def dashboard():
    ...
    return render_template('db.html', name=name, gender=gender, age=age)
