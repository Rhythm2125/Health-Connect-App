...
def problem_solving():
    return render_template('psdb.html', quiz=quizzes)
...
def quiz(title):
    ...
    if request.method == "POST":
        total_score = 0
        max_score = len(random_questions) * max(response_scores.values())
        
        # Calculate total score based on user responses
        for i, question in enumerate(random_questions):
            answer = request.form.get(f"question_{i}")
            if answer in response_scores:
                total_score += response_scores[answer]
        
        # Calculate a percentage score or assign a category based on the score
        percentage_score = (total_score / max_score) * 100
        
        return render_template("psr.html", score=total_score, percentage=percentage_score)

    return render_template("psq.html", quiz=quiz, questions=random_questions)
